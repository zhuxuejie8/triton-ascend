...
@triton.jit
def triton_fma_4d_5d(
        output_ptr, x_ptr, y_ptr, z_ptr,
    ...
    z_val = tl.load(z_ptr + offsets, masks)
    ret = tl.fma(x_val, y_val, z_val)
    tl.store(output_ptr + offsets, ret, mask=masks)


@pytest.mark.parametrize('shape', TestUtils.full_shape)
@pytest.mark.parametrize('dtype', ['float32', 'float16', 'bfloat16'])  # math.fma do not support int dtype
def test_fma(dtype, shape):
    ...
    ans = (
        x.cpu().to(torch.float64)
        * y.cpu().to(torch.float64)
        + z.cpu().to(torch.float64)
    ).to(x.dtype)
...
def test_fma_4d_5d(shape, dtype):
    logging.log(logging.DEBUG, f"shape = {shape}")
    x = test_common.generate_tensor(shape, dtype).npu()
    y = test_common.generate_tensor(shape, dtype).npu()
    z = test_common.generate_tensor(shape, dtype).npu()
    ...
    grid = (1,)
    triton_fma_4d_5d[grid](output, x, y, z, *blocks, *blocks, *strides)
