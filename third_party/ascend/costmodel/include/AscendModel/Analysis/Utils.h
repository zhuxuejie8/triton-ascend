...
inline LoopTripCountResult getScfForTripCountWithBindings(
    scf::ForOp forOp,
    const llvm::DenseMap<unsigned, int64_t> &argBindings = llvm::DenseMap<unsigned, int64_t>(),
    const llvm::StringMap<int64_t> &programIdBindings = llvm::StringMap<int64_t>()) {
...
public:
  ...
  void exportProgramIdBindings(llvm::StringMap<int64_t> &out) const {
    for (const auto &kv : bindings_) {
      llvm::StringRef key = kv.first();
      if (key.starts_with("pid_") || key.starts_with("program_id_") || key.starts_with("num_programs") ||
          key == "x" || key == "y" || key == "z") {
